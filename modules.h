...
public:
    ...
    /// (n_vocab, n_embed).
    Embedding(const Tensor& weight, const Tensor& emb_acv, const Tensor& proj_acv);
...
public:
    ...
    PosEmbedding(const Tensor& weight, int max_ctx);
...
public:
    ...
    LayerNorm(const Tensor& weight, const Tensor& bias, const Tensor& acv);
...
public:
    ...
    Linear(const Tensor& weight, const Tensor& bias, const Tensor& acv);
    Tensor forward(const Tensor& inp);
    int64_t time() const noexcept { return time_ms_; }
...
public:
    ...
    MultiHeadSelfAttn(const Linear& query, const Linear& key, const Linear& value,
                      const Linear& out_proj, const Tensor& qk_acv, const Tensor& qkv_acv,
                      int n_head);
    Tensor forward(const Tensor& inp);
